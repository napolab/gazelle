...
class GazeLLE(nn.Module):
    ...
    def forward(self, input):
        ...
        head_maps = torch.cat(self.get_input_head_maps(input['bboxes']), dim=0).to(
            x.device
        )  # [sum(N_p), 32, 32]
        ...
        x = x.reshape(x.shape[0], self.featmap_h, self.featmap_w, x.shape[2]).permute(
            0, 3, 1, 2
        )  # b (h w) c -> b c h w
    ...
    def load_gazelle_state_dict(self, ckpt_state_dict, include_backbone=False):
        ...
        if len(keys1 - keys2) > 0:
            print(
                'WARNING provided state dict does not have values for keys: ',
                keys1 - keys2,
            )
