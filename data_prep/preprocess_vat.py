...
def smooth_by_conv(window_size, df, col):
    ...
    padded_track = np.concatenate(
        [
            values[0].repeat(window_size // 2),
            values,
            values[-1].repeat(window_size // 2),
        ]
    )
    ...
    for split in splits:
        ...
        for seq_path in glob.glob(os.path.join(PATH, 'annotations', split, '*', '*')):
            ...
            sample_image = os.path.join(
                PATH, seq_img_path, os.listdir(os.path.join(PATH, seq_img_path))[0]
            )
            ...
            merged_df = merge_dfs(
                person_dfs
            )  # merge annotations per person for same frames
            for frame_idx, row in merged_df.iterrows():
                ...
                for i in range(1, num_ppl * 6 + 1, 6):
                    if not np.isnan(
                        row.iloc[i]
                    ):  # if it's nan lack of continuity (one person leaving the frame for a period of time)
                        xmin, ymin, xmax, ymax, gazex, gazey = row[
                            i : i + 6
                        ].values.tolist()
